...
use crate::cli::handlers::handle_query;
use crate::infra::{FsConfigStore, KeychainSecretStore};

pub async fn run(cli: Cli) -> anyhow::Result<()> {
    let console = Console::new();
    let secret_store = KeychainSecretStore::new("sqel".to_string());
    ...
    match cli.command {
        args::Command::Conn(conn_sub) => match conn_sub.command {
            ...
            ConnectionSubcommand::List(args) => handlers::handle_list(&conn_app, &args).await,
        ...
        Command::Query(args) => handle_query(&console, &conn_app, &args).await
    }
}