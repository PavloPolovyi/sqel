...
impl Config {
    ...
    pub fn add(&mut self, conn: Connection, set_default: bool, overwrite: bool) -> Result<(), ConfigError> {
        let name = conn.name().as_str().to_string();
    ...
    pub fn remove(&mut self, name: &ConnectionName) -> Result<Connection, ConfigError> {
        let conn = self.connections.remove(name.as_str()).ok_or(ConfigError::NotFound)?;
        if self.default_connection.as_deref() == Some(name.as_str()) {
    ...
    pub fn get(&self, name: &ConnectionName) -> Option<&Connection> {
        self.connections.get(name.as_str())
    }
    
    pub fn set_default(&mut self, name: &ConnectionName) -> Result<(), ConfigError> {
        if self.connections.contains_key(name.as_str()) {
            self.default_connection = Some(name.as_str().to_string());
            return Ok(());
        }
        Err(ConfigError::NotFound)
    }
    
    pub fn unset_default(&mut self) -> Option<String> {
